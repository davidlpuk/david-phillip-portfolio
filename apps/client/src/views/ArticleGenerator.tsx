...
import NovelEditor, { prepareForNovel } from "@/features/cv-pdf/NovelEditor";
...
export default function ArticleGenerator() {
    ...
    useEffect(() => {
        if (editSlug) {
            console.log('Loading article for editing, slug:', editSlug);
            setLoading(true);
            setIsEdit(true);

            // First try to load from regular articles
            getArticle(editSlug)
                .then(article => {
                    ...
                    if (article) {
                        ...
                        console.log('Set form data from regular article');
                    } else {
                        console.log('Article not found in regular articles, trying drafts...');
                        // If not found in regular articles, try drafts
                        return fetch(`http://localhost:3001/api/articles/drafts/${editSlug}`)
                            .then(response => {
                            ...
                            .then(draft => {
                                ...
                                setFormData({
                                    ...
                                    date: draft.importedAt || draft.publishDate || new Date().toISOString().split('T')[0],
                                    readTime: draft.readingTime || 5,
                                    thumbnail: '',
                                    slug: draft.slug,
                                    featured: false,
                                    status: draft.status || 'draft',
                                    htmlContent: draft.markdown || '', // Draft markdown content
                                });
                                console.log('Set form data from draft');
                            });
                    }
                })
                .catch(error => {
    ...
    const generateMarkdown = async () => {
        ...
        try {
            ...
            console.log('Form data:', {
                ...
                hasContent: !!formData.htmlContent
            });
...
${markdownContent}`;
            ...
            console.log('Frontmatter created, length:', frontmatter.length);
            ...
            alert(`Error generating or saving article: ${errorMessage}`);
        } finally {
            setGenerating(false);
        }
    };
